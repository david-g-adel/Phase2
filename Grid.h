...
public:
	...
	void PrintMessageAndWait(string msg); // Prints an error message on statusbar, Waits for mouse click then clears statusbar
									    // We added this function once here because it is used many times by other classes

	~Grid(); // A destructor for any needed deallcations
	Cell* GetCellFromCellPos(CellPosition);
	int LadderCount();
	int SnakeCount();
	int CardCount();
	void ClearAll();
	void SaveAll(ofstream &OutputFile, Object);
	bool IsOverLapping(GameObject* newObj);
	GameObject* getObj(CellPosition& pos);
};
