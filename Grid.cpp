...
void Grid::PrintMessageAndWait(string msg)
{
...
Cell* Grid::GetCellFromCellPos(CellPosition C)
{
...
void Grid::SaveAll(ofstream &OutputFile, Object objType)
{
	for (int i = NumVerticalCells - 1; i >= 0; i--)
	{
		for (int j = 0; j < NumHorizontalCells; j++)
		{
			...
			{
				CellList[i][j]->GetGameObject()->Save(OutputFile,objType);
			}
		}
	}
}
...
GameObject* Grid::getObj(CellPosition& pos)
{
	int i = pos.VCell();
	int j = pos.HCell();
		if (CellList[i][j]->GetGameObject() != NULL)
