...
Output::Output()
{
	...
	UI.CellColor_NoCard = LIGHTBLUE;
	UI.CellNumFont = 13;
	UI.CellNumColor = BLACK;

	// Cell Color if Has Card & CARD Number Font & Color
	UI.CellColor_HasCard = GOLDENROD;
	UI.CardNumFont = 35;
	UI.CardNumColor = BLACK;

	// Ladder Line Width and Color
	UI.LadderlineWidth = 6;
	UI.LadderColor = DARKSLATEGREY;
...
int Output::GetCellStartX(const CellPosition& cellPos) const
{
	...
	return x;///TODO: implement the following function as described in Output.h file
...
{
	int y;
	y = (cellPos.VCell()) * UI.CellHeight + UI.CellHeight;
	return y;///TODO: implement the following function as described in Output.h file
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\clear.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ROLL_DICE] = "images\\roll.jpg";
	...
	MenuItemImages[ITM_EXITPLAY] = "images\\exit.jpg";
	///TODO: Prepare images for each menu item and add it to the list
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	for (int i = fromStartY;i > y2;i = i - UI.CellHeight)
	{
